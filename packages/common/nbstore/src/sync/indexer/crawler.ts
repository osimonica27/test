...
import { IndexDocument } from '../../storage';
...
function getOrCreateCachedYDoc(data: Uint8Array) {
  try {
    const hash = toHexString(digest(data));
...
export async function crawlingDocData({
  ...
  block: IndexDocument<'block'>[];
  preview?: IndexDocument<'docPreview'>;
}> {
  ...
  const ydoc = getOrCreateCachedYDoc(docBuffer);
  let docTitle = '';
  ...
  return {
    block: blockDocuments.map(block =>
      IndexDocument.from<'block'>(`${docId}:${block.blockId}`, {
        docId: block.docId,
    ...
    preview: IndexDocument.from<'docPreview'>(docId, {
      docId,
      summary,
    }),
  };
...
export function crawlingRootDocData({
  allIndexedDocs,
  rootDocBuffer,
}: {
  ...
  allDocs: IndexDocument<'doc'>[];
  deleted: string[];
} {
  const ydoc = getOrCreateCachedYDoc(rootDocBuffer);
  ...
  const availableDocs = new Map<string, { title: string }>();
  ...
  const availableDocsIds = Array.from(availableDocs.keys());

  const needDelete = difference(allIndexedDocs, availableDocsIds);

  return {
    deleted: needDelete,
    allDocs: availableDocsIds.map(docId => {
      const doc = availableDocs.get(docId);
      if (!doc) {
        throw new Error('Unreachable');
      }
      return IndexDocument.from<'doc'>(docId, {
        docId: docId,
        title: doc.title,
      });
    }),
