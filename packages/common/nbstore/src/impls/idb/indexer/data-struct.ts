...
export class DataStruct {
  ...
  constructor() {
    for (const [tableName, table] of Object.entries(IndexerSchema)) {
      ...
      for (const [fieldName, type] of Object.entries(table)) {
        const typeInfo: IndexFieldSchema =
  ...
  private async insert(
    ...
    const dataMap =
      exists && isUpsert
        ? new Map([...exists.data, ...document.fields]) // merge exists data with new data
        : document.fields;

    const nid = await trx.objectStore('indexerRecords').put({
      table,
      id: document.id,
      data: dataMap,
    });
  ...
  async batchWrite(
    ...
    table: keyof IndexerSchema,
    deletes: string[],
    inserts: IndexDocument<any>[],
    updates: IndexDocument<any>[]
  ) {
    for (const del of deletes) {
      await this.delete(trx, table, del);
    }
    for (const inst of inserts) {
      await this.insert(trx, table, inst, false);
    }
    for (const update of updates) {
      await this.insert(trx, table, update, true);
    }
  }
  ...
  async getAllIds(
    ...
    return (
      await trx.objectStore('indexerRecords').index('table').getAll(table)
    ).map(r => r.id);
  }
