...
export const SelfHostTeamCard = () => {
  ...
  }, [permission, workspaceQuotaService, workspaceSubscriptionService]);

  const description = useMemo(() => {
    const expirationDate = workspaceSubscription?.end
      ? new Date(workspaceSubscription.end).toLocaleDateString()
      : '??';
    if (isTeam) {
      return t[
    ...
    if (isLocalWorkspace) {
      return 'If you want to activate the license for this workspace, please sign in and enable cloud.';
    }
    return t[
  ...
  }, [isLocalWorkspace, isTeam, t, workspaceQuota, workspaceSubscription]);
