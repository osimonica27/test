...
function printAbsolutePaths(schema: BuiltRouteSchema, level = 1): string {
  ...
  if (schema.children.length) {
    absolutes.push(`root: '${schema.fromRoot}'`);
  } else {
  ...
  return level === 1 ? `export const ABSOLUTE_PATHS = ${output}` : output;
}
...
function printRelativePaths(schema: BuiltRouteSchema, level = 1): string {
  ...
  if (schema.children.length) {
    relatives.push(`root: '${schema.fromParent}'`);
  } else {
  ...
  return level === 1 ? `export const RELATIVE_PATHS = ${output}` : output;
}
...
function printFactories(schema: BuiltRouteSchema): string {
  ...
  if (!schema.parent) {
    ...
    return `${output}\nexport const PATH_FACTORIES = { ${firstLevelNames.join(
      ', '
    )} };`;
  }
